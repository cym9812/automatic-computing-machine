...
def orangesRotting1(grid) -> int:
    m = len(grid)
    n = len(grid[0])
    direction = [[1, 0], [0, 1], [-1, 0], [0, -1]]
    visited = [[False] * n for y in range(m)]
    bad = [(x, y) for x in range(n) for y in range(m) if grid[y][x] == 2]
    count = 0
    while True:
        next_bad = []
        while bad:
            ...
            for d in direction:
                x_d = x + d[0]
                y_d = y + d[1]
                if -1 < x_d < n and -1 < y_d < m:
                    if not visited[y_d][x_d]:
                        if grid[y_d][x_d] == 1:
                            visited[y_d][x_d] = True
                            grid[y_d][x_d] = 2
                            next_bad.append([x_d, y_d])
        if not next_bad:
            break
        bad = next_bad
        count += 1
    return -1 if [1 for y in range(m) for x in range(n) if grid[y][x] == 1] else count

orangesRotting1([[2,1,1],[0,1,1],[1,0,1]])
